	...
	const SDL_Rect *dest_rect, SDL_Surface *dest_surface, int softscale, int scanlines, int rotated)
{
 ...
 if(rotated == MDFN_ROTATE90)
 {
  ...
  {
   uint32 *scol = src_surface->pixels + (src_rect->y * src_surface->pitchinpix) + src_rect->x + y;
 ...
 else // Plain copy
 {
  ...
  uint32 src_pitch_diff = src_surface->pitchinpix;
  uint16 *drow = dpixels;
  uint32 *srow = src_surface->pixels + (src_rect->y * src_surface->pitchinpix) + src_rect->x;
