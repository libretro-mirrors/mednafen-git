// Might be necessary over DX5 for 64-bit builds for x86_64 Windows, but we shall see(also need to investigate purported potential lag that DX8 has over DX5)
#error "No pie for you."
...
#define DIRECTINPUT_VERSION 0x0800 //zero 29-oct-2012 - change to dinput8
#define _WIN32_WINNT 0x0501  //zero 29-oct-2012 - this file demands XP featureset
#include <windows.h>
#include <windowsx.h>
#include <dinput.h>
...
class Joystick_DX8 : public Joystick
{
 public:

 Joystick_DX8(LPDIRECTINPUT dii, DIDEVICEINSTANCE *ddi);
 virtual ~Joystick_DX8();
...
void Joystick_DX8::RequestExclusive(bool value)
{
...
Joystick_DX8::Joystick_DX8(LPDIRECTINPUT dii, DIDEVICEINSTANCE *ddi) : dev(NULL), have_exclusive_access(-1)
{
...
void Joystick_DX8::SetRumble(uint8 weak_intensity, uint8 strong_intensity)
{
 //if((weak_intensity || strong_intensity) && rumble_supported)
 // RequestExclusive(true);
}

class JoystickDriver_DX8 : public JoystickDriver
{
 public:

 JoystickDriver_DX8(bool exclude_xinput);
 virtual ~JoystickDriver_DX8();
...
{
 HMODULE us32 = NULL;
 UINT  (WINAPI *p_GetRawInputDeviceList)(PRAWINPUTDEVICELIST, PUINT, UINT) = NULL; //zero 29-oct-2012 - change location of WINAPI to make compile in msvc
 UINT  (WINAPI *p_GetRawInputDeviceInfo)(HANDLE, UINT, LPVOID, PUINT) = NULL; //zero 29-oct-2012 - change location of WINAPI to make compile in msvc
 std::set<uint32> exclude_vps;

 if((us32 = LoadLibrary("user32.dll")) == NULL)
  return exclude_vps;

 p_GetRawInputDeviceList = (UINT  ( WINAPI*)(PRAWINPUTDEVICELIST, PUINT, UINT))GetProcAddress(us32, "GetRawInputDeviceList"); //zero 29-oct-2012 - change location of WINAPI to make compile in msvc
 p_GetRawInputDeviceInfo = (UINT  (WINAPI*)(HANDLE, UINT, LPVOID, PUINT))GetProcAddress(us32, "GetRawInputDeviceInfoA"); //zero 29-oct-2012 - change location of WINAPI to make compile in msvc
 ...
 if(p_GetRawInputDeviceList && p_GetRawInputDeviceInfo)
 {
  ...
  unsigned int num_devices = 0;
	...
	p_GetRawInputDeviceList(NULL, &alloc_num_devices, sizeof(RAWINPUTDEVICELIST)); //zero 29-oct-2012 - added sizeof parameter to end
...
static BOOL CALLBACK GLOB_EnumJoysticksProc(LPCDIDEVICEINSTANCE ddi, LPVOID private_data)
{
 ...
 if((ddi->dwDevType & 0xFF) != DI8DEVCLASS_GAMECTRL)//zero 29-oct-2012 - change DIDEVTYPE_JOYSTICK to DI8DEVCLASS_GAMECTRL
...
JoystickDriver_DX8::JoystickDriver_DX8(bool exclude_xinput) : dii(NULL)
{
 ...
 try
 {
  REQUIRE_DI_CALL( DirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&dii, NULL) ); //zero 29-oct-2012 - change to use dinput8 and add void** cast
  REQUIRE_DI_CALL( dii->EnumDevices(DI8DEVCLASS_GAMECTRL, GLOB_EnumJoysticksProc, &edl, DIEDFL_ATTACHEDONLY) ); //zero 29-oct-2012 - change DIDEVTYPE_JOYSTICK to DI8DEVCLASS_GAMECTRL
  ...
  for(unsigned i = 0; i < edl.valid_count; i++)
  {
   ...
   try
   {
    jdx5 = new Joystick_DX8(dii, &edl.ddi[i]);
    joys.push_back(jdx5);
   }
...
JoystickDriver_DX8::~JoystickDriver_DX8()
{
...
unsigned JoystickDriver_DX8::NumJoysticks(void)
{
 return joys.size();
}

Joystick *JoystickDriver_DX8::GetJoystick(unsigned index)
{
 return joys[index];
}

void JoystickDriver_DX8::UpdateJoysticks(void)
{
 for(unsigned int n = 0; n < joys.size(); n++)
 {
...
JoystickDriver *JoystickDriver_DX8_New(bool exclude_xinput)
{
 return new JoystickDriver_DX8(exclude_xinput);
}
