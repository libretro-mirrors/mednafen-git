...
template<int flag, int value> void SMP::op_branch() {
  rd = op_readpc();
...
template<int mask, int value> void SMP::op_bitbranch() {
  dp = op_readpc();
  sp = op_readdp(dp);
  rd = op_readpc();
  op_io();
...
void SMP::op_jmp_addr() {
  rd  = op_readpc() << 0;
  rd |= op_readpc() << 8;
  regs.pc = rd;
}

void SMP::op_jmp_iaddrx() {
  dp  = op_readpc() << 0;
  dp |= op_readpc() << 8;
  op_io();
  dp += regs.x;
...
template<int n> void SMP::op_tcall() {
  dp  = 0xffde - (n << 1);
  rd  = op_readaddr(dp + 0) << 0;
  rd |= op_readaddr(dp + 1) << 8;
  op_io();
